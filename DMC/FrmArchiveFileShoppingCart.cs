...
namespace DMC
{
    ...
    /// 档案区文件购物车
    /// </summary>
    public partial class FrmArchiveFileShoppingCart : BaseForm
    {
        //树形结构上面的右键菜单
        private ContextMenuStrip stripTreeView = new ContextMenuStrip();
        //那规则就是双击节点加载目录使用，点击节点右侧加载列表，使用+-号算点击对应的节点
        private _QueryApprovalProjectStructure _queryInfo = null;
        private string projectId = null;
        public FrmArchiveFileShoppingCart()
        {
            InitializeComponent();

            _queryInfo = new _QueryApprovalProjectStructure()
            {
                fileType = 0, //文件来源0 项目区 1归档区
                type = 0,    //发起类型 0购物车 1文件夹 2项目 3文件
                fileIds = "",  //流id列表  用  ，分割
                parentId = "0",  //上级ID
                tab = "1"      //是否获得未归档的文件 0未归档 1全部 默认传1，就出版和下载传 0  剩下的都传1
            };
        }
        ...
        private void FrmArchiveFileShoppingCart_Load(object sender, EventArgs e)
        {
            LoadFileList();
        }
        ...
        private void openFile(object sender, EventArgs e)
        {
            ...
            var frm = new FrmPreviewArea(projectInfo.filePath,_queryInfo.fileType, listUrl);
            frm.Show();
        }
        ...
        private void LoadFileList()
        {
            ...
            if (HttpPost(AppGlobalModel.GetApprovalProjectStructure,_queryInfo, ref resultData))
            {
                if (resultData != null && resultData.Any())
                {
                    ...
                    if (_queryInfo.parentId == "0")
                    {
                        ...
                        if (HttpPost(AppGlobalModel.GetApprovalProjectStructureAll,_queryInfo, ref resultFileAllData))
                        {
        ...
        private void treeView1_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    _queryInfo.parentId = selectInfo.id;
        ...
        private void btn发起审批_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    this.Close();
                    var frm = new FrmInitApproval(resultData, 0, "", 1);//传递流程参数：resultdata：项目属性；0：购物车；“”，1：档案区
                    frm.ShowDialog();
                }
                #endregion
            }
    ...
    class _QueryApprovalProjectStructure
    {
        /// <summary>
        /// 文件来源0 项目区 1归档区
        /// </summary>
